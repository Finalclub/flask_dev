...
class UploadForms(FlaskForm):
	...
	# result_list = session.query(Datatype.id_dtype, Datatype.name).order_by(Datatype.id_dtype).all()
...
class SelectDSForm(FlaskForm):
	dataset = core.SelectField(
		# coerce=int,
		# choices=session.execute('select id, name from dataset where dataset_type=1').fetchall()
		choices=SQLHelper.fetch_all('select id, name from dataset where dataset_type=1')
	)
	labels = core.SelectField(
		# coerce=int,
		# choices=session.execute('select id, name from dataset where dataset_type=2').fetchall()
	...
	def __init__(self):
		...
		# self.dataset.choices = session.execute('select id, name from dataset where dataset_type=1').fetchall()
		# self.dataset.choices = session.execute('select id, name from dataset where dataset_type=2').fetchall()
